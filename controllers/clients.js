...
const updateClient = async (req, res, next) => {
    try {
        ...
        .then((result) => {
            ...
            res.setHeader('Content-Type', 'application/json');
            res.send({json: {status: 200, message: 'Client updated successfully'}});
        })
    } catch (error) {
        console.log(error.message);
        next(error);
...
module.exports = { getAllClients, createClient , updateClient, findByPhone, findBySSID, deleteClient};